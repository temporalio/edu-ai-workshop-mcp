...
11. To register a new tool in Claude Desktop when running MCP servers in Codespaces, restart the MCP server in Codespaces. Do this by going to the terminal window where you're running `uv run mcp_servers/weather.py`, stop it with `Ctrl+C`, then run it again.
